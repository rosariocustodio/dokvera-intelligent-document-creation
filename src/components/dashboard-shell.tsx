...
function CreditsBadge({ userId }: { userId: string }) {
  const { data: balance = 0 } = useQuery(creditsQuery(userId));
  return (
    <Link
      to="/credits"
    ...
    >
      ...
      <p className="mt-1 font-display text-2xl font-bold">{balance}</p>
      <p className="text-xs opacity-85">{formatMzn(creditsToMzn(balance))}</p>
    </Link>
  );
}
