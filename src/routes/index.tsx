...
import {
  ...
  creditsToMzn,
  formatMzn,
  packPriceMzimport { useState } from "react";
import { createFileRoute, Link } from "@tanstack/react-router";
import {
  ArrowRight,
  BookOpen,
...
const description =
  "O Dokvera ajuda-te a criar, organizar e preparar documentos académicos e profissionais com tecnologia de IA. Sistema de créditos simples e transparente, disponível em vários países.";
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <div className="flex items-center gap-2">
            <Select value={country} onValueChange={(value) => setCountry(value as CountryCode)}>
              <SelectTrigger className="h-9 w-[110px] rounded-xl text-xs" aria-label="País e moeda">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {listCountries().map((c) => (
        ...
        <div className="relative mx-auto max-w-6xl px-5 pt-20 pb-16 md:pt-28 md:pb-24">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-5 text-xs text-muted-foreground">
              1 crédito = {formatCurrency(countryConfig.creditPrice, country)} · 10 créditos de boas-vindas na primeira
              conta
            </p>
          </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="gradient-brand shadow-glow rounded-2xl p-5 text-brand-foreground">
                  ...
                  <p className="mt-1 text-xs opacity-85">
                    = {formatCurrency(creditsToCurrency(32, country), country)}
                  </p>
                </div>
                {[
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {DOCUMENT_TYPES.map((t) => (
            ...
            >
              ...
              <p className="mt-5 text-xs text-muted-foreground">
                ≈ {formatCurrency(creditsToCurrency(t.cost, country), country)}
              </p>
            </div>
          ))}
        </div>
      ...
      <section id="precos" className="scroll-mt-20 border-y border-border/60 bg-surface py-20">
        <div className="mx-auto max-w-6xl px-5">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-3 font-display text-3xl font-bold md:text-4xl">
              Pagas o que usas — 1 crédito = {formatCurrency(countryConfig.creditPrice, country)}
            </h2>
            <p className="mt-4 text-muted-foreground">
              Sem mensalidades. Compras créditos, usas quando precisas e vês sempre o equivalente na tua
              moeda.
            </p>
          </div>
